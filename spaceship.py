...
def removeElement(alist,index):
    '''This function takes in a list and an index and removes the
    element from that index'''
    tmp = [x for x in alist if x!=alist[index]]
    return tmp
...
while True:
    ...
    if scene.kb.keys:
        ...
        if keyPressed == 'up':
            ...
            lazer = sphere(pos = (0,0,80), radius = 0.2, color = color.red)
            lazers += [lazer]
            #if lazerCount > 8:
            lazerCount += 1 
            lazers[lazerCount-1].pos.z += -1
            #lazerCount += 1
